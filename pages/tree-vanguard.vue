...
<script lang="ts" setup>
  ...
  import data from '~/assets/game_versions/latest/tech_trees/1v1/Vanguard_tech_tree.json';
  

  const graphExtents: CoordinateExtent = [[-250, -250], [1250, 2000]];

  onMounted(() => {
    document.title = "Vanguard Tech Tree | SGTome";
  ...
  // these are our nodes
  const nodes = ref<Node[]>(data.nodes);
  const edges = ref<Edge[]>(data.edges);
